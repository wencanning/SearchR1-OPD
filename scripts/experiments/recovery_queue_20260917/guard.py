"""Run one owned worker on physical GPU3; preserve shared retriever and others."""
import json
import os
...
def free():
    ...
    idx,mem=map(int,row.strip().split(','));assert idx==3
    return mem
...
def main():
    ...
    if free()<32*1024:raise RuntimeError('GPU3 startup free memory below32GiB; not launched')
    ...
    try:
        with (out/'gpu_watchdog.jsonl').open('a') as f:
            while p.poll() is None:
                ...
                if mem<16*1024 or own>=26*1024:raise RuntimeError('GPU3 memory guard; stop ONLY our worker')
                time.sleep(1)
