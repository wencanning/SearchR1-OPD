...
SOURCE=ROOT/'reports/recovery_continue_gpu4_20260917/continue_student'
...
def main():
    ...
    if (QUEUE/'DISABLE_GPU3').exists():
        allowed_tokens.add('USER_CANCELLED_GPU3\n')
    if not stop.exists() or stop.read_text() not in allowed_tokens:
    ...
    old=json.loads((QUEUE/'queue_resume_after_gpu4.json').read_text())['queue_pid']
    for _ in range(8):
        try:
    ...
    if args.collect_exit==0:
        ...
        assert status['state']=='complete' and status['completed']==status['total']==len(inp['cases'])*inp['replicates']==512
        assert len(rows)==512 and {(r['qid'],r['replicate']) for r in rows}=={(c['qid'],k) for c in inp['cases'] for k in range(inp['replicates'])}
        ...
        dest=QUEUE/'continue_student';dest.mkdir(exist_ok=True)
        copies={}
        ...
        save(dest/'GPU4_REUSE_PROVENANCE.json',dict(source=str(SOURCE),sha256=copies,
            scope='Actual student continuations on GPU4 copied byte-for-byte, not a second independent sample. GPU3 skips student collection and continues with teacher only after luorongchuan exits.'))
        save(dest/'status.json',dict(status,student_continuations_on_gpu4=True))
        record.update(imported=True,continuations=len(rows))
    # The parallel GPU4 teacher must finish (or fail) before this resume, so the
    # original GPU3 queue cannot launch duplicate teacher continuations.
    if (QUEUE/'gpu4_teacher_dependency.json').exists():
        subprocess.run([str(ROOT/'.venv/bin/python'),
            str(ROOT/'scripts/experiments/recovery_continue_teacher_gpu4_20260917/await_and_import.py')],
            cwd=ROOT,check=True)
    if not stop.exists() or stop.read_text() not in allowed_tokens:
        raise RuntimeError('Handoff pause token changed while waiting; do not auto-resume')
    # When both GPU4 collections finished, CPU analysis needs no GPU3 slot.
    teacher_status=QUEUE/'continue_teacher/status.json'
    if record['imported'] and teacher_status.exists() and json.loads(teacher_status.read_text()).get('state')=='complete':
        subprocess.run([str(ROOT/'.venv/bin/python'),
            str(ROOT/'scripts/experiments/recovery_queue_20260917/summarize.py')],cwd=ROOT,check=True)
    if (QUEUE/'DISABLE_GPU3').exists():
        record.update(phase='gpu4_handoff_finished_gpu3_disabled',queue_pid=None,
                      reason='User cancelled GPU3 queue; STOP remains; no GPU3 fallback')
        save(QUEUE/'gpu4_continuation_handoff.json',record)
    ...
    save(QUEUE/'queue_resume_after_gpu4_continuations.json',record)
    save(QUEUE/'gpu4_continuation_handoff.json',record)
    print(json.dumps(record,indent=2),flush=True)
