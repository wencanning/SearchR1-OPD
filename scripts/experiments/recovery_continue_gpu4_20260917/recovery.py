"""Actual same-state student/teacher continuation pilot on separate train/eval Qs."""
import argparse
import hashlib
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--phase',choices=['continue'],required=True);p.add_argument('--model',choices=['student'],required=True);p.add_argument('--out',type=Path,required=True);args=p.parse_args()
    assert os.environ.get('CUDA_VISIBLE_DEVICES')==GPU and os.environ.get('RECOVERY_GPU_GUARDED')=='1'
    assert args.phase!='collect' or args.model=='student'
    import torch
    from transformers import AutoModelForCausalLM,AutoTokenizer,StoppingCriteria,LogitsProcessor
    import requests
    torch.set_num_threads(4);torch.set_num_interop_threads(1);torch.cuda.set_per_process_memory_fraction(8*1024**3/torch.cuda.get_device_properties(0).total_memory)
    ...
    manifest=dict(phase=args.phase,model=args.model,source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),code_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),checkpoint=str(path),weights=weights,gpu=GPU,retriever='http://127.0.0.1:8000/retrieve')
    if args.phase=='continue':
        sf=ROOT/'reports/recovery_queue_20260917/collected/states.jsonl'
    ...
    if args.phase=='continue':
        statefile=ROOT/'reports/recovery_queue_20260917/collected/states.jsonl';collected=[json.loads(l) for l in statefile.read_text().splitlines()];stateby={r['qid']:r for r in collected}
        status=json.loads(statefile.with_name('status.json').read_text());assert status['state']=='complete'
        cases=[c for c in cases if stateby[c['qid']]['eligible'] and (args.model=='student' or c['split']=='train')]
