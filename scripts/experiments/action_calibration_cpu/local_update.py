...
def roles(tok,ids):
    ...
    tags=list(re.finditer(r'</?(?:think|search|answer|information)>',text))
    actions=[m.span(1) for m in re.finditer(r'<(search|answer)>',text)]
    lengths=[0]+[len(tok.decode(ids[:i],skip_special_tokens=False)) for i in range(1,len(ids)+1)]
    result=[]
    for a,b in zip(lengths,lengths[1:]):
        hits=[(s,e) for s,e in actions if a<e and b>s]
        if hits:
            result.append('action' if any(a>=s and b<=e for s,e in hits) else 'action_mixed')
        elif any(a<m.end() and b>m.start() for m in tags):result.append('format')
...
def update(m,torch):
    ...
    for variant in cfg['variants']:
        ...
        for c in samples:
            ...
            elif variant=='body_only_er':active=torch.tensor([float(role=='body') for role in c['roles']])
        ...
        done.add(variant);save(OUT/'status.json',dict(state='running',stage='local_update',completed=len(done),total=4,cuda_initialized=False))
        print('LOCAL_UPDATE_COMPLETE',variant,'grad_norm',grad,'seconds',round(time.time()-tic,1),flush=True)
        del opt;gc.collect()
    assert len(rows(OUT/'update_effects.jsonl'))==4
