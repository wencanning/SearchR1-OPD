"""Ordinary teacher demonstrations from original TRAIN questions on GPU4."""
import argparse
import hashlib
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--phase',choices=['question_start'],required=True);p.add_argument('--model',choices=['teacher'],required=True);p.add_argument('--out',type=Path,required=True);args=p.parse_args()
    assert os.environ.get('CUDA_VISIBLE_DEVICES')==GPU and os.environ.get('RECOVERY_GPU_GUARDED')=='1'
    assert args.phase=='question_start' and args.model=='teacher'
    import torch
    from transformers import AutoModelForCausalLM,AutoTokenizer,StoppingCriteria,LogitsProcessor
    import requests
    torch.set_num_threads(4);torch.set_num_interop_threads(1);torch.cuda.set_per_process_memory_fraction(20*1024**3/torch.cuda.get_device_properties(0).total_memory)
    source=ROOT/'reports/recovery_queue_20260917/train_inputs.frozen.json';protocol=json.loads(source.read_text());cases=protocol['cases']
    assert len(cases)==128 and all(c['split']=='train' for c in cases) and protocol['replicates']==4
    ...
    manifest=dict(phase=args.phase,model=args.model,source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),code_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),checkpoint=str(path),weights=weights,gpu=GPU,scope='Question-start teacher demonstrations, train-only, bounded diagnostic decoder, not full training',retriever='http://127.0.0.1:8000/retrieve')
    ...
    def search(query):
        ...
        return obs
    result=args.out/'continuations.jsonl'
    ...
    reps=protocol['replicates'];total=len(cases)*reps
    for c in cases:
        for rep in range(reps):
            if (c['qid'],rep) in done:continue
            seed=int(hashlib.sha256(f'2026091747|{args.phase}|{c["qid"]}|{rep}'.encode()).hexdigest()[:8],16);torch.manual_seed(seed);torch.cuda.manual_seed_all(seed);start=time.monotonic()
            ids=tok.apply_chat_template(c['prompt'],tokenize=True,add_generation_prompt=True)
