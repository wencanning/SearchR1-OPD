"""Teacher continuation of the frozen 128 actual student TRAIN states on GPU4."""
import argparse
import hashlib
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--phase',choices=['continue'],required=True);p.add_argument('--model',choices=['teacher'],required=True);p.add_argument('--out',type=Path,required=True);args=p.parse_args()
