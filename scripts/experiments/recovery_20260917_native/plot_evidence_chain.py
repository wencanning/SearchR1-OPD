...
import numpy as np
ROOT=Path(__file__).resolve().parents[3];DATA=ROOT/'reports/recovery_20260917_native';OUT=ROOT/'figures/recovery_20260917_native'


def main():
    update=json.loads((DATA/'update_summary.json').read_text());rec=json.loads((DATA/'recovery_summary.json').read_text())
    paths=[ROOT/'reports/recovery_20260917/probe_teacher/answers.jsonl',DATA/'recovery_summary.json',DATA/'update_summary.json']
    ...
    def rate_bars(ax,groups,colors):
        for x,(name,rs) in enumerate(groups):
            ...
            records.append(dict(panel=ax.get_title(loc='left'),label=name.replace('\n',' '),correct=k,n=n,percent=y,low=lo,high=hi))
    ...
    for x,(v,c) in enumerate(zip(vals,[grey,pink,blue])):
        ...
        records.append(dict(panel=b.get_title(loc='left'),label=['All tested states','Student failed both attempts','Teacher recovered'][x],correct=v,n=vals[0],percent=100*v/vals[0],low=None,high=None))
    ...
    for arm,label in zip(arm_names,arm_labels):
        p=DATA/('continue_student' if arm=='zero' else 'eval_'+arm)/'continuations.jsonl';paths.append(p)
        assert json.loads(p.with_name('status.json').read_text())['state']=='complete'
        groups.append((label,[r for r in map(json.loads,p.read_text().splitlines()) if r['split']=='eval']))
        assert len(groups[-1][1]) == update['arms'][arm]['n']
        assert sum(r['em'] for r in groups[-1][1]) == update['arms'][arm]['correct']
