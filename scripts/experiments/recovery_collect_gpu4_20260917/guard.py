...
def main():
    ...
    if free()<24*1024:raise RuntimeError('GPU4 startup free memory below24GiB; not launched')
    ...
    try:
        with (out/'gpu_watchdog.jsonl').open('a') as f:
            while p.poll() is None:
                ...
                if mem<16*1024 or own>=10*1024:raise RuntimeError('GPU4 memory guard; stop ONLY our worker')
                time.sleep(1)
