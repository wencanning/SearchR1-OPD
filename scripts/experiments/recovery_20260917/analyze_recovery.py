...
def main():
    ...
    result=dict(n_input_train=16,n_eligible_train=len(per),n_input_eval=8,student_train=arm([r for r in values['student'] if r['split']=='train']),teacher_train=arm(values['teacher']),student_eval=arm([r for r in values['student'] if r['split']=='eval']),
      ...
      qualification='Pilot. Two failed student samples do not establish impossibility. Teacher outcome verification does not prove student learnability, evidence grounding, or transfer. Train and eval question-disjoint. No oracle evidence used.')
    ...
    report+=['',f"学生两次均失败：{result['observed_student_all_failed']}题；其中教师至少一次成功：{result['teacher_rescued_questions']}题；可用成功教师后缀：{len(targets)}条。",'资格只是目标是否存在；不能替代训练后新题生成。收集第一查询成本另计，不能宣称完整训练/交互预算已经匹配。']
