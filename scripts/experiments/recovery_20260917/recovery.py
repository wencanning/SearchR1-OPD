...
def main():
    p=argparse.ArgumentParser();p.add_argument('--phase',choices=['collect','continue'],required=True);p.add_argument('--model',choices=['student','teacher'],required=True);p.add_argument('--out',type=Path,required=True);args=p.parse_args()
    ...
    source=ROOT/'reports/recovery_20260917/train_inputs.frozen.json';protocol=json.loads(source.read_text());cases=protocol['cases']
    ...
    if args.phase=='continue':
        sf=ROOT/'reports/recovery_20260917/collected/states.jsonl'
    ...
    def generate(ids,budget):
        ...
        valid=bool(match and match[2].strip() and not re.search(r'</?(?:think|search|answer|information)>',match[2]))
    ...
    if args.phase=='continue':
        statefile=ROOT/'reports/recovery_20260917/collected/states.jsonl';collected=[json.loads(l) for l in statefile.read_text().splitlines()];stateby={r['qid']:r for r in collected}
    ...
    for c in cases:
        for rep in range(reps):
            ...
            initial=list(ids);segments=[];calls=0;used=0;answer=None;stop=None
            while True:
                ...
                if g['action']!='search':stop='invalid_or_incomplete';break
