...
TEACHER_BASE_URL="${TEACHER_BASE_URL:-http://127.0.0.1:8004/v1}"
TEACHER_MODEL="${TEACHER_MODEL:-SearchR1-qwen2.5-3b-grpo-v0.2}"
TEACHER_TOKENIZER="${TEACHER_TOKENIZER:-Qwen/Qwen2.5-3B-Instruct}"
RETRIEVER_URL="${RETRIEVER_URL:-http://127.0.0.1:8000/retrieve}"

DATA_SOURCES="${DATA_SOURCES:-hotpotqa,nq}"
SPLIT="${SPLIT:-train}"
SAMPLES_PER_SOURCE="${SAMPLES_PER_SOURCE:-30000,10000}"   # 单个数字（所有数据源相同）或逗号分隔列表（一一对应 DATA_SOURCES）
SEED="${SEED:-7}"
